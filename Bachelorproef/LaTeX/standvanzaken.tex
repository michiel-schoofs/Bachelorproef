...
De doelstelling van deze bachelorproef is om een volledig gedecentraliseerd versiebeheer systeem te bekomen. Zoals in de vorige sectie reeds werd uitgelegd is een versiebeheersysteem een verzameling van bestanden en een logboek waarin alle veranderingen worden bijgehouden. Dit logboek is in essentie niets meer of minder dan een collectie van metadata en kunnen we bijgevolg gaan decentraliseren aan de hand van een blockchain (hier wordt verder op ingegaan in hoofdstuk 3). \textbf{Maar dan blijft de vraag nog steeds waar kunnen de bestanden worden bewaard?}\\

In eerste instantie zou men kunnen denken dat deze simpelweg ook op de blockchain kunnen worden gezet maar dit is niet mogelijk. In tweede instantie zou er eventueel kunnen worden aangenomen dat deze bestanden gewoonweg kunnen worden verstuurd tussen bob en alice. Ook deze oplossing is niet ideaal aangezien Bob en Alice dan in het klassieke server-client scenario terecht komen. Er is dus een nood aan een manier om op een gedecentraliseerde manier bestanden te gaan delen met elkaar. Dit principe wordt ook wel \textbf{gedecentraliseerde file-sharing} genoemd.\\

Er bestaan verschillende protocollen die deze doelstelling voor ogen hebben.  Eerst volgt een algemene uitleg van verschillende begrippen om voldoende achtergrond informatie te geven om deze protocollen te begrijpen. Vervolgens wordt een overzicht gegeven van drie verschillende protocollen: Bit-Torrent, Gnutella en IPFS.\\

Waarom wordt er juist geopteerd voor deze drie protocollen? Bit-Torrent en Gnutella zijn een mooi voorbeeld van twee verschillende types van protocollen. Bij Bit-Torrent is er namelijk nog spraken van een zekere mate van centralisatie -zie \ref{torrenting}-. Gnutella is volledig gedecentraliseerd maar heeft ook een aantal beperkingen -zie \ref{Gnutella}. Tot slot komen we tot een meer recent protocol dat veel van de beperkingen van de vorige twee protocollen gaat opvangen. Dit protocol wordt IPFS genoemd en is het protocol dat wordt gebruikt in de POC.\\
