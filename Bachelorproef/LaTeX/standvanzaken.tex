...
\begin{itemize}
	\item Een nadeel van dit centraal punt is het zogenaamde SPOF probleem -zie \ref{vb_conclusie}-. Doordat alle informatie zich op één server bevind is er ook één kritieke plaats waar alles kan fout lopen. Is de server onbeschikbaar zijn alle diensten en informatie hierop dat ook. Om dit probleem op te lossen kan gebruik worden gemaakt van zogenaamde \textit{Mirror servers}. Deze servers zijn exacte kopieën
	
	\item Doordat al het verkeer moet verwerkt worden door dezelfde hardware kan dit leiden tot een hoge serverbelasting. Hierdoor kan het verwerken van deze verzoeken traag verlopen.
...
\end{itemize}
 ...
 Dit lijkt in eerste instantie een contradictie. Zoals aangehaald binnen \ref{vb_inleiding} bestaat er reeds het concept van DVCS \textit{Distributed version control system}. Hierbij beschikt elke computer over een lokale kopie van het archief. Op het eerste zicht 