...
Het Torrent protocol heeft duidelijk een aantal voordelen. Zoals eerder aangehaald is -ondanks dat het al sinds 2002 geformaliseerd- het nog steeds relevant. Het biedt immers een relatief eenvoudige manier aan om bestanden te gaan downloaden. Door een goede metainformatie indexeringswebsite te gebruiken zoals \textit{The pirate bay} vindt men op eenvoudige wijze de correcte bestanden terug. Dit is bij FTP minder evident.\\
...
\item Er is weinig stimulans voor een peer die bestanden download om deze ook toegankelijk te stellen voor het netwerk en de rol van Seeder op zich te gaan nemen. In sommige gevallen wordt om dit probleem te gaan een zogenaamde \textit{Tit for tat} beleid doorgevoerd. Hierbij kunnen peers enkel bestanden gaan downloaden indien ze effectief ook bijdragen aan de download van anderen.\\
...
Merk op dat veel van deze problemen ook aanwezig zijn bij het klassieke client-server model.
