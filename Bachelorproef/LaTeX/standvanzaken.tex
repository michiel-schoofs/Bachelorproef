...
Versiebeheer is geen nieuw concept. Er zijn zoals eerder aangehaald verschillende software oplossingen beschikbaar. Volgens \textcite{Chacon2014} zijn er drie grote categorieën (zie \ref{fig:TypesVCS} voor een grafische weergave):
...
Al deze concepten worden ook nog eens grafisch verduidelijkt in de figuur \ref{fig:tree}.
...
In de vorige paragraaf wordt aangehaald, dat elke versie exact één kind heeft. Dit is echter niet de volledige realiteit. Er is eveneens het concept van \textbf{takken} (eng: branches). Net zoals stambomen kunnen vertakken, kan dit ook in het archief. Dit principe kan men het best visualiseren. In grafiek \ref{fig:deltas} kan men zien dat het project vertakt is aan versie 1.3. De hoofdtak of versie die van de wortel afstamt wordt de \textbf{trunk} genoemd. Op eerste zicht kan dit complex en onnodig lijken. \textcite{Tichy85rcs} geeft enkele redenen om dit toch toe te passen:
...
De mogelijkheid om toch een stabiele versie te hebben terwijl men veranderingen kan aanbrengen verklaard het succes van vertakken(en: \textit{branching}). Het is een efficiënte manier om geïsoleerd te kunnen samenwerken. Stel een project met honderden bestanden en verschillende ontwikkelaars. In zo een omgeving kunnen veranderingen soms onvoorspelbare gevolgen hebben. Ontwikkelaars kunnen het project vertakken, wijzigen en testen alvorens men het in productie neemt. Zo kan men neveneffecten beperken en heeft men altijd een productie waardige code op de hoofdtak.
...
De doelstellingen van deze bachelorproef is om een werkend P2P versiebeheer systeem te gaan implementeren. Hiervoor moet men afbakenen welke functionaliteiten deze implementatie moet voorzien. In vorige paragrafen werden de verschillende concepten besproken. Hieronder volgt een overzicht van deze verschillende concepten alsook een korte uitleg. Hierbij worden de terminologie en concepten van Git gebruikt. Deze worden vervolgens meegenomen naar volgende hoofdstukken, waar een implementatie wordt voorzien:
